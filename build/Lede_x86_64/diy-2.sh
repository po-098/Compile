...
sed -i "/uci commit network/i\uci set network.lan.ipaddr='192.168.31.126'" $ZZZ              # IPv4 地址(openwrt后台地址)
...
sed -i "/uci commit system/i\uci set system.@system[0].hostname='Sunny-126'" $ZZZ       # 修改主机名称为OpenWrt-123

sed -i "s/OpenWrt /Sunny build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ        # 增加自己个性名称281677160
